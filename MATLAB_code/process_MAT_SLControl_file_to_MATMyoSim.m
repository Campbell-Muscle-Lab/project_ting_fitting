function out = process_MAT_SLControl_file_to_MATMyoSim(td,varargin)
% Takes a SLControl mat file and generates a protocol file
...
if (~isequal(slc_dt,p.t_inc))
    ...
    td.fibre_length = interp1(td.time,td.force,new_time)';
    td.time = new_time';
end

% Smooth fl
if (p.smooth_fl_points>0)
    td.fibre_length = smoothdata(td.fibre_length,'movmean',p.smooth_fl_points);
end
...
if (~isempty(p.protocol_file_string))
    ...
    % Generate dhsl
    normalized_fl = td.fibre_length(ti)./td.fibre_length(ti(1));
...
if (p.figure_number)
    ...
    subplot(r,c,2);
    plot(t_disp, td.fibre_length(ti), 'b-');
    ylabel('FL');
    xlim([0 max(t_disp)]);
